...
class User(db.Model):
    ...
    settings = db.Column(JSON, nullable=False, default=list)
...
def register():
    if request.method == 'POST':
        ...
        newUser.settings = STARTER_SETTINGS.copy()
...
# get settings data
@app.route('/game/<username>/getSettings')
def getSettings(username):
    user = User.query.filter_by(username=username).first()
    return jsonify(user.settings or [])

# Save settings data
@app.route('/game/<username>/saveSettings', methods=['POST'])
def saveSettings(username):
    data = request.get_json()
